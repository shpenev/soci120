...
# If you want to know what directory R is treating as its home directory, here's the command:

getwd()

#ls() lists all the objects in your local workpace (You can also see them all in the Environment panel of RStudio)

ls()

# dir() or list.files() are used to list all the files in your working directory. Both commands have the same result.

dir()

# dir.create() creates a new directory. Don't forget the quotation marks around the directory name.

dir.create("soci120")

# You can change the working directory with change.dir() command.

change.dir("soci120")

# You can create a new file using file.create()

file.create("Homework01.R")

# You can rename files using file.rename()

file.rename("Homework01.R", "hw01.R")

# You can copy files using file.copy()

file.copy("hw01.R", "hw01_backup.R")

# you can create directories using dir.create()

dir.create(file.path("Homeworks", "hw01"), recursive = T)
